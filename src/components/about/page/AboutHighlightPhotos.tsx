...
import { FALLBACK_ABOUT_HIGHLIGHT_PHOTOS } from '../../../core/content/aboutPage';
import { useSanityQuery } from '../../../hooks/useSanityQuery';
import { fetchAboutHighlightPhotos } from '../../../lib/sanity/queries';

const GOLD = '#d4af37';

const AboutHighlightPhotos = ({ className = '' }: { className?: string }) => {
  const { data: photos } = useSanityQuery(
    fetchAboutHighlightPhotos,
    FALLBACK_ABOUT_HIGHLIGHT_PHOTOS,
  );
    ...
    >
      {photos.map((photo) => (
        ...
        >
          ...
          <p
            className={`absolute bottom-2.5 left-3 right-3 font-mono uppercase tracking-[0.12em] ${
              photo.fit === 'contain' ? 'text-center' : ''
            }`}
            style={{
              fontSize: '0.88rem',
              color:
